const steps = ['Facility Profile', 'Licensing & Classification', 'Infrastructure & Services', 'Contacts & SPOCs', 'Systems & Interoperability', 'Documents', 'Verification', 'Review & Submit']; let cur = 0; let tickets = [{ id: 'QNDHM-TKT-2026-0112', reg: 'FAC-2026-00125', subject: 'Sandbox authentication support', category: 'Integration', priority: 'High', status: 'Open', agent: 'Integration Support', sla: 'Within SLA', owner: 'registrant', events: [{ title: 'Ticket Created', date: '28 Aug 2026', note: 'Ticket generated through QNDHM portal' }, { title: 'Assigned', date: '28 Aug 2026', note: 'Assigned to Integration Support' }] }, { id: 'QNDHM-TKT-2026-0098', reg: 'FAC-2026-00125', subject: 'Additional authorization document required', category: 'Documents', priority: 'Medium', status: 'Resolved', agent: 'Registration Support', sla: 'Met', owner: 'registrant', events: [{ title: 'Ticket Created', date: '24 Aug 2026', note: 'Document request raised' }, { title: 'Resolved', date: '25 Aug 2026', note: 'Required document received and reviewed' }] }, { id: 'QNDHM-TKT-2026-0074', reg: 'OTHER-2026-00044', subject: 'Compliance observation clarification', category: 'Compliance', priority: 'Low', status: 'In Progress', agent: 'Compliance Team', sla: 'Within SLA', owner: 'other', events: [{ title: 'Ticket Created', date: '20 Aug 2026', note: 'Compliance clarification requested' }] }];
...
// function showStep(i) { cur = Math.max(0, Math.min(i, steps.length - 1)); document.querySelectorAll('.tab').forEach(x => x.classList.remove('active')); document.getElementById('t' + cur).classList.add('active'); document.querySelectorAll('.step').forEach((x, j) => x.classList.toggle('active', j === cur)); document.getElementById('stepTitle').textContent = steps[cur]; document.getElementById('bar').style.width = ((cur + 1) / steps.length * 100) + '%'; scrollTo(0, 0) } function move(d) { showStep(cur + d) } function saveDraft() { let n = new Date().toLocaleString(); localStorage.setItem('QNDHM_Healthcare_Facility_Role_Based.html', n); document.getElementById('saved').textContent = 'Saved locally: ' + n }
function showStep(i) {
